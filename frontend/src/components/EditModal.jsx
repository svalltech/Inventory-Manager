...
const EditModal = ({ item, isCreateMode, brands, warehouses, productTypes, categories, sizes, colors, materials, designs, weights, productHierarchy: propProductHierarchy, onClose, onSave }) => {
  ...
  const handleAddVariants = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const allItems = await response.json();
        // Filter items that match: same brand, category, and product name (across ALL warehouses)
        const allVariants = allItems.filter(item => {
        ...
        console.log('Variants:', allVariants.map(v => `${v.sku} (${v.size}) - ${v.warehouse}`));
        setExistingVariants(allVariants);
      } else {
        setExistingVariants([]);
      }
    ...
    newVariants.forEach((variant, index) => {
      ...
      if (variant.size && variant.warehouse) {
        ...
        if (newSizeWarehouseCombos.includes(combo)) {
          ...
          // Check against existing variants for this specific warehouse
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {!formData.product_type ? (
                  ...
                  <div className="w-full px-3 py-2 border border-slate-300 rounded-lg bg-amber-50 text-amber-700 text-sm">
                    No categories for this product type. Please add in Settings → Product Hierarchy
                  </div>
                ) : (
                  <select
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  ...
                  <span className="text-xs font-normal text-blue-600 ml-2">● Auto-generated</span>
                </label>
                <input
                  type="text"
                  name="sku"
                  ...
                  required
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg bg-slate-100 text-slate-600 font-mono cursor-not-allowed"
                />
                <p className="text-xs text-slate-500 mt-1">
                  Format: Brand-Warehouse-Type-Category-Name-Material-Design-Color-Size
                </p>
              </div>
                    ...
                    <table className="w-full border border-slate-300 rounded-lg">
                      ...
                      <tbody>
                        {newVariants.map((row, index) => {
                          // Get the selected warehouse for this row
                          const selectedWarehouse = row.warehouse;
                          
                          // Get sizes that exist in the SELECTED warehouse only
                          ...
                          // Combine both to get disabled sizes for this warehouse
